...
>I think what happened is that you have two phrases copied, and when you paste, R pastes the two phrases together. As we talked about in class, it's like adding and dividing math equations.
...
>The == means that the values on the left are the same as the values on the right, which in the case of the first example is not true. As with the example above, 3 is less than 4, so that is true. And again, in the last instance, 3 is supposed to equal the value of 4 or 3. 
...
```{r}
my.calculation <- (x + x)
my.data <- (numberone * numbertwo)

my.calculation + my.data
```
...
>First, we created a list with George Washington; then we added more names to the list. When we use length, we see how many names/items are in the list. If we want the value of an item, we can type that into myvalue. And then if the values are numbers, we can multiply them within the list.
...
>We got the value of the 3rd number in the numbers list that we then multiplied by 2.

(@) You try, create a list of five items and store it in a descriptive variable. 
```{r}
mycohort <-c("Justin", "David", "Jared", "Carrie", "Anne")
```
...
R is the language of choice for most data scientists and that is because of its powerful suite of data analysis tools. Some are built into R, like the `floor()` which we looked at above. But others come from packages. Most programming languages have some sort of package system although every language calls it a slightly different thing. Ruby has gems, python has eggs, and php has libraries. In R these are called packages or libraries and they are hosted by the Comprehensive R Archive Network or more commonly, CRAN. CRAN is a network of ftp and web servers around the world that store identical, up-to-date, versions of code and documentation for R. You didn't know it but you already used CRAN when you looked up the documentation for `floor()` above. If you go to the CRAN webpage and look at the [list of available R packages](https://craln.r-project.org/web/packages/available_packages_by_name.html) you'll see just how many there are!

So there are many packages but there are also some that are indispensable and that you'll use over and over for this class. We'll get into some of those in the next two worksheets but for now lets look at one basic package for data. 
...
< I had to Google, but you simply ask R to look at the "head" or first couple of entries, or look at the "tail," or the last couple of entries. 
...
<Gayguides is the dataset, the $ is the operator that R uses to search the dataset, the title is the entry in the title columns, and the [500] is looking for the 500th entry or value in the dataset.
...
head(BostonWomenVoters, n=6L)
```
data("BostonWomenVoters")
tail(BostonWomenVoters), n=6L)
...
<Boston Women Voters has around 10,702 observations of 43 variables. There's information about name, precicnt, occupation, and tons of information about the woman's husband's name, county, court of naturalization, and the woman's father's name, country, county, on and on. It seems wild that there is so much information about the men married to or related to these women. 


CONGRATS! You've completed your first worksheet for Digital Methods II. That wasn't so bad, right? 

It wasn't! I'm sorry I'm turning this in so late - as you know, dev.tools wasn't working for me until today's class. Thank you for being so helpful!
