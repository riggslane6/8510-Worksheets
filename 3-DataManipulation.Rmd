...
```
> How many women have served in the U.S. District Courts since 1970? 
...
judges %>%
  filter(grep("U.S. District Court", paste("Court.Type..1", "Court.Type..2.")))%>%
  group_by(Gender) %>%
  summarize(District.Court = n()) 
...
almshouse.discharged %>%
  select("Year.Admitted," "YearDischarged") %>%
  pivot_wider(names_from = Year.Admitted, values_from = count) %>%
  mutate(growth = YearDischarged - Year.Admitted)
...
gayguides.filters <- function(States, year) {

gayguides %>%
  select(type, state, Year, title) %>%
  filter(state == States && Year == year) %>%
  group_by(type, state, Year, title)
}

gayguides.filters("CA", 1990)
```
