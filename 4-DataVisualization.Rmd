...
guides.risk <- gayguides %>%
  filter(grepl("(HOT)" | "(AYOR)", amenityfeatures) | grepl("inquire locally", description) | grepl("inquire locally", streetaddress) | grepl("inquire", title)) %>%
  group_by(Year) %>%
  summarize(count = n())
...
#I tried to facet wrap Year before type and it was not clear at all what the data was. type is much clearer.
...
workers.rec <- rec.workers %>%
  ...
  mutate(volunteer_average = mean(c(male.volunteer, women.volunteer, volunteer))) %>%
  select(city, Year, paid.average, volunteer_average) 
